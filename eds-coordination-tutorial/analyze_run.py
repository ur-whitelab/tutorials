...
def plot_bias(eds_files,labels, N, M=8):
    '''plot the biases'''
    ...
    for i, (e, l) in enumerate(zip(eds_files, labels)):
        ...
        for (i,j), ax in np.ndenumerate(axes):
            try:
                ...
                name = column_labels[i * M + j + 1]
            except IndexError:
                continue
            col = data[:, i * M + j + 1]#add one to skip time
            #for means, remove 0s
            if(name.split('_')[-1] == 'mean' or name.split('_')[-1] == 'std'):
                ax.plot(data[col != 0, column_dict['time']] / 1000, col[col != 0], label=l)
            else:
                ax.plot(data[:, column_dict['time']] / 1000, col, label=l)
            if i != N - 1:
